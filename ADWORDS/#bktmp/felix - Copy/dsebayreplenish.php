<?php
	...
	$fh = fopen(REPLENISH_EB_DS , 'w');
	fwrite($fh,$fileheader);
	fclose($fh);


	//get listings from active listings table at 0

	$active_final = $dbase_getters->get_ds_active_listings_stock_over_x(0);
	...
	for ($i=0; $i < sizeof($active_final[EB_ACT_ITEM_ID])  ;$i++){
		...
		if (sizeof($item_final[ITEM_STOCK_LEVEL]) > 0){
			...
			if (($kerp_stock_level >= $min_eb_stock) && ($kerp_onmarket == ITEM_ON_MARKET_STATUS_SET)){
