	...
	if (isset($_POST['uploadfile']) || isset($_POST['dlfile'])){
		...
		$display = "<div class='wrappermain'>";
			...
			$display .= "<div class='leftmenu'>";
				...
				$pagex[0][0] = "Manage Listings";
				$pagex[0][1] = "http://localhost/KOKO_LIVING_EBAY_API/ebay_api_update_listing/";
				

				$display .= $html_output->list_element($pagex,false,1);
				...
				$pageb[0][0] = "Download Active Variation Listings on Ebay";
				$pageb[0][1] = "ebay.php?import=4";
			...
			if (!isset($_GET['import'])){
				...
				$newsarray = array();
				$newsarray[0]['date'] = "Hello";
				$newsarray[0]['news'] = "<ul><li>Welcome to eBay management centre.</li></ul>";
...
///////////////////////////////////import Active Variation listings from ebay
				if ($_GET['import'] == 4){
					...
					if (isset($_POST['uploadfile'])){
						...
						require_once "getlistingsvar.php";



						$display .= "<div class='alertok'>";
							$display .= "<p>Active Variation listings successfully downloaded.</p><p>You may download the active listings file <a href='".ACTIVE_VAR_OUTPUT_HTTP."'>HERE</a></p>";
						$display .= "</div>";

						/*//update time
						$cols[0][0] = EB_UPD_ACT_VAR_LIST;
						$cols[0][1] = time();
						$dbase_setters->update_query(EB_UPD_TRACKING, $cols);*/
					...
					$display .= "<div class='newshow'>";
						$display .= "<h2>Download Active Variation Item Listings.</h2>";
						$display .= "<p>All current active listings which contain variations will be downloaded from eBay and entered into the database and saved to a file.</p>";

						$form_elements = $html_output->hidden_element("uploadfile", 1);
						$form_elements .= "<p>" .$html_output->create_submit("Download Active Variation Listings") ."</p>";

						$display .= "<fieldset>" . $html_output->create_form(0, "ebay.php?import=4", $form_elements) . "</fieldset>";
...
$lastupdate = 'xxx';
						...
						$pagedr[1][0] = "<b>eBay Active VARIATION Listings</b>  ____ (Last Update ____<i>" . date("d-M-Y____H:i:m", $lastupdate[EB_UPD_ACT_VAR_LIST][0]) ."</i>)";
						$pagedr[1][1] = ACTIVE_VAR_OUTPUT_HTTP;
