<?php
	...
	$fh = fopen(ACTIVE_VAR_OUTPUT, 'w') or die("can't open file");
	fclose($fh);
	...
	echo "Download Active Variation Listings... Please Wait.<br/>";
	//write header to file
	$fh = fopen(ACTIVE_VAR_OUTPUT, 'a') or die("can't open file");
	...
	do {
		...
		$item_add_return_array = $dom_functions->get_listings_var_details($dom);
		...
		$currentfirstnum = $item_add_return_array['itemnumber'][0];
		$prev_first_item_number = updateItem::$prev_first_item_number;
		
		//don't continue if first item numbers are identical
		if ($currentfirstnum == $prev_first_item_number){
			
			//echo "SAME SAME SAME " . $currentfirstnum . " --- " . $prev_first_item_number . " <br/>";
			break;
			
		}
		else{

			updateItem::$prev_first_item_number = $currentfirstnum;
		
		
			for ($x=0; $x < $num_of_listings ;$x++){

				//compare item numbers for pagination. Do not continue if first item number already exists.
		//GETMYEBAYSELLING_ENTRIES_PER_PAGE
				...
				//only continue if the item is a variation item
				if ($item_add_return_array['varnodecount'][$x] > 0){

					//Strings to separate with ## delim
					$ebay_quantity_tp = $item_add_return_array['varquantity'][$x];	
					$varitemtitle_tp = $item_add_return_array['varitemtitle'][$x];
					$ebay_sku_tp = $item_add_return_array['varcustomlabel'][$x];
					$price_tp = $item_add_return_array['varsellprice'][$x];
					$quantity_sold_tp = $item_add_return_array['quantitysoldandavailable'][$x];

					$ebay_quantity_tp_bits = explode(STRING_DELIM, $ebay_quantity_tp);
					$varitemtitle_tp_bits = explode(STRING_DELIM, $varitemtitle_tp);
					$ebay_sku_tp_bits = explode(STRING_DELIM, $ebay_sku_tp);
					$price_tp_bits = explode(STRING_DELIM, $price_tp);
					$quantity_sold_tp_bits = explode(STRING_DELIM, $quantity_sold_tp);


					
					//go through individual item variations
					for ($d=0; $d < $item_add_return_array['varnodecount'][$x]; $d++){
					

				///////////////////////////enter into database
						$ebay_itemnumber = $item_add_return_array['itemnumber'][$x];

						$quantity_sold = $quantity_sold_tp_bits[$d]; 
						$ebay_quantity = $ebay_quantity_tp_bits[$d];
						$varitemtitle = $varitemtitle_tp_bits[$d];
						$ebay_sku = $ebay_sku_tp_bits[$d];
						$price = $price_tp_bits[$d];
						...
						$ittitle = str_replace("	","",trim($varitemtitle));
...
/*
						if ($tempx > 0){
							...
							$active_insert_cols[0][0] = EB_ACT_ITEM_IDVAR;
							$active_insert_cols[0][1] = $ebay_itemnumber;
							$active_insert_cols[1][0] = EB_ACT_SKUVAR;
							$active_insert_cols[1][1] = $ebay_sku;
							$active_insert_cols[2][0] = EB_ACT_QUANTITYVAR;
							$active_insert_cols[2][1] = $ebay_quantity;
							$active_insert_cols[3][0] = EB_ACT_PURCHASESVAR;
							$active_insert_cols[3][1] = $quantity_sold;
							$active_insert_cols[4][0] = EB_ACT_PRICEVAR;
							$active_insert_cols[4][1] = $price;
							$active_insert_cols[5][0] = EB_ACT_START_DATEVAR;
							$active_insert_cols[5][1] = $startdate['start'];
							$active_insert_cols[6][0] = EB_ACT_END_DATEVAR;
							$active_insert_cols[6][1] = $enddate;
							$active_insert_cols[7][0] = EB_ACT_TITLEVAR;
							$active_insert_cols[7][1] = $ittitle;
							$active_insert_cols[8][0] = EB_ACT_OS_CONTROLVAR;
							$active_insert_cols[8][1] = $stock_cntrl_binary;
							$active_insert_cols[9][0] = EB_ACT_STOCK_TYPEVAR;
							$active_insert_cols[9][1] = $stock_type;
							$active_insert_cols[10][0] = EB_ACT_DROPSHIP_IDVAR;
							$active_insert_cols[10][1] = $ds_id;
							$active_insert_cols[11][0] = EB_ACT_LIST_TYPEVAR;
							$active_insert_cols[11][1] = $list_type_add;


							$dbase_setters->insert_query(EB_ACT_EBAY_ACTIVEVAR, $active_insert_cols);
						}
*/
						/*else{
							if ($ebay_quantity > 0){
				...
				*/
				...
				///////////////////////////////create the output file
						...
						$stringoutput .= $ebay_sku . TAB_DELIM;
						$stringoutput .= $ebay_quantity . TAB_DELIM;
