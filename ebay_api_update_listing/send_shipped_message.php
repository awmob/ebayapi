<?php
	...
	*/
	...
	//read the input file
	$fileread = file_get_contents(SHIPPEDMSG);
	//separate into lines
	$file_bits = explode(NS,$fileread);
	echo "<h2>Send Shipped Item Message</h2>";
	...
	for ($i=0; $i < sizeof($file_bits) ;$i++){
		...
		if (trim($file_bits[$i]) != ""){
			...
			$item_id = $file_segments[0];
			$username = $file_segments[1];
			$message = $file_segments[2];


			//DISPLAY INFO
			$linenumber = $i+1;
			echo "Line " . $linenumber . " : <b>ITEM ID#</b> " . $item_id . "... <b>USERNAME:</b> " . $username . "<p>".$message."</p>"; 
		
			//END DISPLAY INFO
			$xml_request = $updateItem->sendshippedmsg($auth_token, $item_id, $username, $message);

			//Create headers to send with CURL request.
			$headers = $call_functions->headersmain($compat_level, $dev_id, $app_id, $cert_id, CALL_ADDMEMBERMSG, AUSTRALIA_SITE_ID);
			...
			$item_add_return_array = $dom_functions->get_msg_sent_details($dom, $username);
			...
			for ($x=0; $x < sizeof($item_add_return_array)  ;$x++){
				...
				echo "<span style='color:red;font-weight:bold;font-size:1.3em;'>" . $item_add_return_array[$x] . "<br/>";
