<?php
	...
	*/
	...
	echo "<h2>Update Ordinary Items</h2>";
	...
	for ($i=0; $i < sizeof($file_bits) ;$i++){
		...
		if (trim($file_bits[$i]) != ""){
			...
			$stock_level = $file_segments[2];
			$item_price = $file_segments[3];
			

			//get the xml for a new auction
			$xml_request = $updateItem->update_price_and_stock($auth_token, $ebay_id, $stock_level, $item_price);

			//Create headers to send with CURL request.
			$headers = $call_functions->headersmain($compat_level, $dev_id, $app_id, $cert_id, CALL_NAME_UPDATEITEM, AUSTRALIA_SITE_ID);
			...
			$item_add_return_array = $dom_functions->get_item_return_details($dom, $sku, $ebay_id);
