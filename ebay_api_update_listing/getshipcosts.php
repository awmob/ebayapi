<?php
	...
	*/
	...
	//read the input file
	$fileread = file_get_contents(FILE_GETSHIPCOSTS);
	//separate into lines
	...
	for ($i=0; $i < sizeof($file_bits) ;$i++){
		...
		if (trim($file_bits[$i]) != ""){
			...
			$xml_request = $reviseItem->get_ship_costs($auth_token, $ebay_id);

			//Create headers to send with CURL request.
			$headers = $call_functions->headersmain($compat_level, $dev_id, $app_id, $cert_id, CALL_GETSHIPCOSTS, AUSTRALIA_SITE_ID);
			...
			$item_add_return_array = $dom_functions->get_shipcost_details($dom, $sku, $ebay_id);
